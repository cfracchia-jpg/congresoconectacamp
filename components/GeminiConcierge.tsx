  ...
  return (
    <>
      <button 
        ...
        className="fixed bottom-48 md:bottom-10 right-6 size-14 bg-primary text-text-dark rounded-full shadow-glow z-40 flex items-center justify-center hover:scale-110 active:scale-95 transition-all overflow-hidden"
      ...
      {isOpen && (
        <div className="fixed inset-0 z-[100] bg-surface/60 backdrop-blur-sm flex items-end md:items-center justify-center p-4">
          <div className="bg-surface w-full max-w-md h-[70vh] rounded-t-3xl md:rounded-3xl border border-white/10 flex flex-col shadow-2xl animate-fade-in-up">
            <div className="p-4 border-b border-white/10 flex justify-between items-center bg-background-page/50 rounded-t-3xl">
              ...
              {messages.map((msg, idx) => (
                <div key={idx} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                  <div className={`max-w-[80%] p-3 rounded-2xl text-sm shadow-sm ${
                    msg.role === 'user' 
                    ? 'bg-primary text-text-dark font-medium rounded-br-none' 
            ...
            <div className="p-4 border-t border-white/10 bg-surface rounded-b-3xl">
              <div className="flex gap-2">
                <input 
                  ...
                  className="flex-1 bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-sm focus:outline-none focus:border-primary/50 text-white placeholder-text-muted"
                />
                <button 
                  onClick={handleSend}
                  disabled={isLoading}
                  className="bg-primary text-text-dark size-12 rounded-xl flex items-center justify-center disabled:opacity-50 hover:bg-white transition-colors shadow-soft"
                >
